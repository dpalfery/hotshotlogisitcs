...
describe('Security Utils', () => {
  ...
  describe('validateInput', () => {
    it('should validate shipment ID format', () => {
      expect(SecurityUtils.validateInput('JOB001SHIP', SECURITY_CONFIG.PATTERNS.SHIPMENT_ID)).toBe(true);
      expect(SecurityUtils.validateInput('job001ship', SECURITY_CONFIG.PATTERNS.SHIPMENT_ID)).toBe(false);
      expect(SecurityUtils.validateInput('JOB-001', SECURITY_CONFIG.PATTERNS.SHIPMENT_ID)).toBe(false);
    });

    it('should validate address format', () => {
      expect(SecurityUtils.validateInput('123 Main St', SECURITY_CONFIG.PATTERNS.ADDRESS)).toBe(true);
      expect(SecurityUtils.validateInput('', SECURITY_CONFIG.PATTERNS.ADDRESS)).toBe(false);
    });
  });

  describe('containsMaliciousContent', () => {
    it('should detect script tags', () => {
      expect(SecurityUtils.containsMaliciousContent('<script>alert("xss")</script>')).toBe(true);
      expect(SecurityUtils.containsMaliciousContent('Normal text')).toBe(false);
    });

    it('should detect javascript: protocol', () => {
      expect(SecurityUtils.containsMaliciousContent('javascript:alert("xss")')).toBe(true);
    });

    it('should detect event handlers', () => {
      expect(SecurityUtils.containsMaliciousContent('onclick=alert("xss")')).toBe(true);
    });
  });
